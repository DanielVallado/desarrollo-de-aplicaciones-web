...
public class CardController {
    ...
    @GetMapping("/cards")
    public ResponseEntity<?> listarTodosCards() {
        List<Card> cards = repository.findAll();
        return ResponseEntity.ok(cards);
    }

    @GetMapping("/cards/{id}")
    public ResponseEntity<?> obtenerCardPorId(@PathVariable Long id) {
    ...
    @PostMapping("/cards")
    public ResponseEntity<?> guardarCard(@Valid @RequestBody Card card) {
        repository.save(card);
        return ResponseEntity.ok(card);
    }

    @PutMapping("/cards/{id}")
    public ResponseEntity<?> actualizarCard(@PathVariable Long id, @Valid@RequestBody Card card) {
    ...
    public ResponseEntity<?> eliminarCard(@PathVariable Long id) {
        Card cardExistente = repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Tarjeta no encontrada con ID: " + id));
